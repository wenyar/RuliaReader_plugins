...
收到“生成某某插件”或新增漫画源插件请求后，必须先做可做性预检，预检通过前禁止创建插件目录、写 `package.json`、生成图标或打包 zip。

预检必须确认：

- 源站首页或列表入口可以通过 Rulia 同类网络请求方式访问，不能只依赖搜索引擎缓存、浏览器人工通过页面或猜测 URL。
- 默认列表接口或列表页能真实返回非空漫画列表，且第一条至少有标题、详情 URL 和封面 URL。
- 搜索接口或搜索页能用实际关键词真实返回非空结果；如果源站确实没有搜索能力，必须在汇报中说明并得到用户接受后才继续。
- 详情页能真实返回标题、简介/元信息、封面和非空章节目录。
- 章节页或章节 API 能真实返回非空图片列表。
- 至少一张封面图和一张章节图能用接近 Rulia 最终加载方式真实请求成功，响应是 `image/*`，且不是防盗链图、占位图、错误图或站点警告图。
- 调用 `getImageUrl` 之后的最终图片 URL 必须能普通直连显示；如果只能靠特殊 Referer、Cookie、一次性 header、浏览器上下文或人工验证才能显示，而宿主最终图片加载链路无法稳定携带这些条件，则判定为不可做。

预检失败处理：

- 如果源站返回 Cloudflare 403、验证码、强登录、强防盗链、地区阻断、图片不能直连、接口需要不可稳定复现的签名，或无法确认列表/详情/章节/图片任一核心链路，则必须停止制作。
- 停止制作时只向用户说明不可做的具体环节、测试过的入口和失败原因；不要生成“待修复”插件，不要打包 zip，不要在根 README 插件列表中加入该源。
- 不允许用空列表、缓存页面、搜索引擎摘要、截图目测、超长 `data:` 图片、外部代理或临时本地代理伪装为可用插件。
...
新增或修复插件时，必须先写本地验收测试并运行通过，再生成或刷新 zip。测试需要模拟 `window.Rulia`，直接调用插件接口，禁止只靠人工目测或单个样例判断可用。新增插件还必须完成“可做性预检”；预检失败时不得进入实现、不得生成插件目录、不得打包 zip。无法联网验证时，不得标记为已验收通过，README 验证样例写 `待补充`，最终汇报明确说明未跑联网测试。
...
1. 先做可做性预检，真实跑通列表、搜索、详情、章节和图片直连链路。
2. 如果预检失败，立即停止制作，汇报不可做原因；不得创建插件目录、不得打包 zip、不得把该源加入根 README。
3. 预检通过后，才在本仓库根目录创建或更新 `plugin.PluginName`。
4. 编写或修正 `package.json`、`index.js`、`README.md`、插件图片资源。
5. 根据源站实现列表、详情、章节图片和图片 URL 接口。
6. 更新 `README.md`，记录接口调用和相关逻辑。
7. 编写或更新插件本地测试，覆盖列表、搜索、详情、章节目录、章节图片和图片可访问性。
8. 运行语法检查、本地测试和必要的联网验收测试；测试失败必须先修复，不得直接打包。
9. 生成或刷新 `plugin.PluginName.zip`。
10. 如果是新增插件，同步更新仓库根目录 `README.md` 的插件列表、状态和注意事项。
11. 汇报改动文件、版本号、测试命令、测试结果和已知限制。
