const BASE_URL = 'https://baozimh.org';
const API_URL = 'https://api-get-v3.mgsearcher.com';
const FALLBACK_COVER = BASE_URL + '/pwa-192x192.png';
...
function makeChapterPayload(data) {
	...
	return 'mgchapter://' + params.toString();
}

function parseMangaList(html) {
	const result = [];
	const seen = {};
	const itemRe = /<a\b[^>]*href=["']([^"']*\/manga\/(?!page\/)[^"']+)["'][^>]*>([\s\S]*?<img\b[^>]*>[\s\S]*?)(?:<\/a>)/gi;
	let match;
...
function parseTitle(html, fallback) {
	return stripTags((html.match(/<h1\b[^>]*>([\s\S]*?)<\/h1>/i) || [])[1])
		|| decodeHtml((html.match(/<meta\b[^>]*property=["']og:title["'][^>]*content=["']([^"']*)["']/i) || [])[1] || '').replace(/\s*-\s*包子漫畫.*$/, '')
		|| stripTags((html.match(/<title>([\s\S]*?)<\/title>/i) || [])[1]).replace(/\s*-\s*包子漫畫.*$/, '')
		|| fallback
		|| '包子漫画';
}
...
function parseChapterPayload(chapterUrl) {
	...
	if (text.indexOf('mgchapter://') === 0) {
		const params = new URLSearchParams(text.replace(/^mgchapter:\/\//, ''));
		return {
...
function extractChapterImages(html) {
	...
	const seen = {};
	const add = (url) => {
		const imageUrl = unwrapProxiedImage(url);
		if (imageUrl && /\/scomic\//i.test(imageUrl) && !seen[imageUrl]) {
			seen[imageUrl] = true;
			images.push(imageUrl);
		}
	};
	html.replace(/(?:src|data-src)=["']([^"']+)["']/gi, (_, url) => {
		add(url);
		return '';
	});
	html.replace(/https?:\/\/[^"'<>\\\s]+\/scomic\/[^"'<>\\\s]+?\.(?:jpg|jpeg|png|webp)(?:\?[^"'<>\\\s]*)?/gi, (url) => {
		add(url);
		return '';
	});
	return images;
	...
	try {
		...
		try {
			...
			if (Array.isArray(images) && images.length) {
				result = images.map(item => defaultImageInfo(resolveChapterImageUrl(item.url, imagesInfo.line)));
			}
		} catch (_) {}
		if (!result.length && payload.chapterPageUrl) {
			const htmlImages = extractChapterImages(await requestText(payload.chapterPageUrl, payload.mangaUrl || BASE_URL + '/'));
			result = htmlImages.map(defaultImageInfo);
		}
		if (!result.length) {
